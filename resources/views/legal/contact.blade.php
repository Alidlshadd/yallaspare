    ...
    <section class="mx-auto w-full max-w-[900px] space-y-8">
            ...
            <div class="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <article class="rounded-xl border border-slate-200/80 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60">
                    <div class="flex items-start gap-3">
                        <span class="mt-0.5 inline-flex h-9 w-9 items-center justify-center rounded-lg bg-white text-primary shadow-sm dark:bg-slate-900">
            ...
            <form method="POST" action="{{ route('legal.contact.send') }}" class="mt-5 space-y-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input id="name" name="name" value="{{ old('name', auth()->user()->name ?? '') }}" required class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 outline-none focus:border-primary/30 focus:ring-4 focus:ring-primary/10 dark:border-slate-700 dark:bg-slate-950 dark:text-white">
                    ...
                    <div>
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email', auth()->user()->email ?? '') }}" required class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 outline-none focus:border-primary/30 focus:ring-4 focus:ring-primary/10 dark:border-slate-700 dark:bg-slate-950 dark:text-white">
                    ...
                    <div>
                        ...
                        <input id="phone" name="phone" value="{{ old('phone', auth()->user()->phone ?? '') }}" class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 outline-none focus:border-primary/30 focus:ring-4 focus:ring-primary/10 dark:border-slate-700 dark:bg-slate-950 dark:text-white">
                    ...
                    <div>
                        ...
                        <select id="topic" name="topic" required class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 outline-none focus:border-primary/30 focus:ring-4 focus:ring-primary/10 dark:border-slate-700 dark:bg-slate-950 dark:text-white">
                ...
                <div>
                    ...
                    <input id="subject" name="subject" value="{{ old('subject') }}" required class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 outline-none focus:border-primary/30 focus:ring-4 focus:ring-primary/10 dark:border-slate-700 dark:bg-slate-950 dark:text-white">
                ...
                <div>
                    ...
                    <textarea id="message" name="message" rows="6" required class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 outline-none focus:border-primary/30 focus:ring-4 focus:ring-primary/10 dark:border-slate-700 dark:bg-slate-950 dark:text-white">{{ old('message') }}</textarea>
                    @error('message') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                </div>
                <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#0a0d55]">
                    {{ __('Send Message') }}
                </button>
            </form>
        </section>
        ...
        <section class="rounded-2xl border border-slate-200/70 bg-white p-7 shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
            ...
            <div class="mt-4 flex items-start gap-3">
                <span class="mt-0.5 inline-flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-primary dark:bg-slate-800">
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" aria-hidden="true">
