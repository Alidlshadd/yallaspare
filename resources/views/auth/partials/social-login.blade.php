        ...
        @foreach ($socialProviders as $provider)
            @if ($provider['key'] === 'google')
                <a
                    ...
                    class="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-slate-700 shadow-sm shadow-slate-900/5 transition duration-200 hover:border-red-200 hover:bg-red-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-100 dark:shadow-black/20 dark:hover:border-slate-500 dark:hover:bg-slate-800 dark:focus-visible:ring-offset-slate-900"
            ...
            @else
                <button
                    type="button"
                    class="inline-flex min-h-11 cursor-not-allowed items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-slate-700 opacity-60 shadow-sm shadow-slate-900/5 dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-100 dark:shadow-black/20"
                    aria-label="{{ $provider['label'] }}"
                    disabled
                >
                    <span class="inline-flex h-5 w-5 shrink-0 items-center justify-center" aria-hidden="true">
                    ...
                    <span class="min-w-0 truncate text-slate-700 dark:text-slate-100">{{ $provider['label'] }}</span>
                    <span class="shrink-0 rounded-full border border-slate-300 px-2 py-0.5 text-[11px] font-semibold text-slate-500 dark:border-slate-600 dark:text-slate-400">{{ __('Coming soon') }}</span>
                </button>
            @endif
        @endforeach
    </div>
