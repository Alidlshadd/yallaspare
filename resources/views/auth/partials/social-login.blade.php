@php
    ...
    $showGoogle = (bool) config('services.social_login.visible')
        && (bool) config('services.google.enabled')
        && filled(config('services.google.client_id'))
        && filled(config('services.google.client_secret'))
        && filled(config('services.google.redirect'))
        && SocialProviders::googleEnabled();

    $showApple = (bool) config('services.social_login.visible')
        && (bool) config('services.apple.enabled')
        && filled(config('services.apple.client_id'))
...
@if ($showSocialLogin)
    <div class="mt-5">
        ...
        <div class="mt-4 grid gap-3 {{ $showGoogle && $showApple ? 'sm:grid-cols-2' : '' }}">
            @if ($showGoogle)
                <a
                ...
                >
                    ...
                    <span class="min-w-0 truncate text-slate-700 dark:text-slate-100">{{ __('Continue with Google') }}</span>
                </a>
            @endif
            ...
            @if ($showApple)
                <a
                    ...
                    class="{{ $buttonBase }} {{ $buttonActive }} hover:border-slate-400 hover:bg-slate-50 dark:hover:border-slate-500 dark:hover:bg-slate-800"
                ...
                >
                    ...
                    <span class="min-w-0 truncate text-slate-700 dark:text-slate-100">{{ __('Continue with Apple') }}</span>
                </a>
            @endif
        </div>
    </div>
