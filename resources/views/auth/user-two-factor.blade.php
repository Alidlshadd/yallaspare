...
>
    ...
    <button
        type="button"
        class="inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:hover:bg-white/10"
    ...
    <form method="POST" action="{{ route('logout') }}" class="mt-4 text-center">
        @csrf
        <button type="submit" class="text-sm font-medium text-slate-500 underline decoration-slate-300 underline-offset-4 transition hover:text-slate-700 dark:decoration-slate-700 dark:hover:text-slate-300">
            {{ __('Sign out instead') }}
        </button>
    </form>
                ...
                @foreach ($channelOptions as $channelKey => $option)
                    ...
                    @if ($option['available'])
                        <form method="POST" action="{{ route('user.two-factor.channel') }}" data-channel-form>
                            ...
                            <button
                                type="submit"
                                class="flex w-full items-center gap-3 rounded-xl border px-3.5 py-3 text-start transition {{ $isCurrent ? 'border-accent/50 bg-accent/10 dark:border-accent/40 dark:bg-accent/10' : 'border-slate-200 hover:border-accent/40 hover:bg-slate-50 dark:border-white/10 dark:hover:border-accent/30 dark:hover:bg-white/5' }}"
                                @disabled($isCurrent)
                                data-channel-submit
                            >
                                <span class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full {{ $isCurrent ? 'bg-accent text-navy' : 'bg-slate-100 text-slate-600 dark:bg-white/10 dark:text-slate-300' }}">
