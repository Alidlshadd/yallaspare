    ...
    <div class="space-y-5 pb-16">
        ...
        <form action="{{ route('checkout.review') }}" method="POST" class="space-y-5">
            ...
            <section class="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_20rem] xl:items-start">
                <div class="space-y-4">
                    <section class="cart-row-in rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm shadow-slate-900/5 dark:bg-slate-900 dark:shadow-black/10 sm:rounded-3xl sm:p-6">
                        <div class="flex items-center gap-2.5">
                            ...
                            <h2 class="text-lg font-semibold tracking-[-0.02em] text-slate-950">{{ __('Saved Delivery Address') }}</h2>
                        </div>

                        @if ($addresses->isEmpty())
                            <div class="mt-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-4 dark:border-slate-700">
                                <p class="text-sm font-semibold text-slate-900">{{ __('No saved address found') }}</p>
                                <p class="mt-1 text-sm text-slate-600">{{ __('Add an address in your account before placing an order.') }}</p>
                                        ...
                                        <span class="min-w-0 flex-1">
                                            <span class="flex flex-wrap items-center gap-2">
                                                ...
                                                @if ($address->is_default)
                                                    <span class="inline-flex rounded-full border border-slate-200/80 bg-white px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-600 dark:bg-slate-900">{{ __('Default') }}</span>
                                                @endif
                                            </span>
                    ...
                    <section class="cart-row-in rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm shadow-slate-900/5 dark:bg-slate-900 dark:shadow-black/10 sm:rounded-3xl sm:p-6" style="animation-delay: 90ms">
                        <div class="flex items-center gap-2.5">
                            <span class="flex h-8 w-8 items-center justify-center rounded-xl bg-primary/5 text-primary dark:bg-slate-800 dark:text-slate-200">
                                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" aria-hidden="true">
                            ...
                            <h2 class="text-lg font-semibold tracking-[-0.02em] text-slate-950">{{ __('Notes (optional)') }}</h2>
                        </div>
                        <textarea
                            id="notes"
                            name="notes"
                    ...
                    <section class="cart-row-in rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm shadow-slate-900/5 dark:bg-slate-900 dark:shadow-black/10 sm:rounded-3xl sm:p-6" style="animation-delay: 180ms">
                        <div class="flex items-center gap-2.5">
                            <span class="flex h-8 w-8 items-center justify-center rounded-xl bg-primary/5 text-primary dark:bg-slate-800 dark:text-slate-200">
                                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" aria-hidden="true">
                            ...
                            <h2 class="text-lg font-semibold tracking-[-0.02em] text-slate-950">{{ __('Coupon Code') }}</h2>
                        </div>
                        <div class="mt-4 flex gap-2">
                            <input
                                ...
                                class="min-w-0 flex-1 rounded-2xl border border-slate-200/80 bg-slate-50 px-4 py-3 text-sm uppercase text-slate-900 outline-none transition duration-200 focus:border-primary/20 focus:bg-white focus:ring-4 focus:ring-accent/10 dark:focus:bg-slate-900"
                        ...
                        @enderror
                        <p class="mt-2 text-xs text-slate-500">{{ __('Coupon discounts are confirmed on the order review screen before payment.') }}</p>
                    </section>
                </div>

                <section class="cart-row-in rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm shadow-slate-900/5 dark:bg-slate-900 dark:shadow-black/10 sm:rounded-3xl xl:sticky xl:top-4" style="animation-delay: 140ms">
                    ...
                    @endphp
                    <div class="mt-4 space-y-1.5 border-t border-slate-100 pt-3 text-xs leading-5 text-slate-500">
