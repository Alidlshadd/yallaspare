    ...
    <div class="mx-auto max-w-[1600px] space-y-5">
            ...
            <div class="flex flex-col gap-4 border-b border-slate-200 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <h3 class="text-lg font-bold text-slate-900">{{ __('OTPIQ inbound webhook') }}</h3>
                    </div>
                </div>
            ...
            <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
                <div class="md:col-span-2 xl:col-span-4">
                    ...
                    <div class="flex min-w-0 gap-2">
                        <div class="min-w-0 flex-1 overflow-x-auto rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 font-mono text-sm font-semibold text-slate-700" dir="ltr">{{ $webhookUrl }}</div>
                        <button type="button" data-copy-webhook-url="{{ $webhookUrl }}" class="inline-flex h-10 shrink-0 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 transition hover:border-green-300 hover:text-green-700 dark:hover:border-green-500/50 dark:hover:text-green-300">
                ...
                @foreach ([
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50/70 px-4 py-3 dark:bg-slate-950/40">
                        <div class="text-[10px] font-bold uppercase tracking-widest text-muted">{{ $label }}</div>
                        <div class="mt-1 flex items-center gap-2 text-sm font-bold text-slate-800"><span class="h-2 w-2 rounded-full {{ $ok ? 'bg-emerald-500' : 'bg-accent' }}"></span>{{ $value }}</div>
                    </div>
                @endforeach

                <div class="rounded-xl border border-slate-200 bg-slate-50/70 px-4 py-3 dark:bg-slate-950/40 xl:col-span-2">
                    <div class="text-[10px] font-bold uppercase tracking-widest text-muted">{{ __('Queue status') }}</div>
                    <div class="mt-1 text-sm font-bold text-slate-800">{{ strtoupper($queueStatus['connection']) }} · {{ $queueStatus['mode'] }}</div>
                ...
                <div class="rounded-xl border border-slate-200 bg-slate-50/70 px-4 py-3 dark:bg-slate-950/40 xl:col-span-2">
                    ...
                    <div class="mt-1 text-sm font-semibold text-slate-700">{{ $lastFailedEvent?->error_message ?? __('No processing error recorded.') }}</div>
        ...
        <section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <form method="GET" action="{{ route('admin.whatsapp.events.index') }}" class="grid gap-3 border-b border-slate-200 p-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
                ...
                <input type="date" name="date_from" value="{{ $filters['date_from'] ?? '' }}" aria-label="{{ __('Start date') }}" class="rounded-xl border-slate-300 bg-white text-sm dark:border-slate-700 dark:text-white">
                <input type="date" name="date_to" value="{{ $filters['date_to'] ?? '' }}" aria-label="{{ __('End date') }}" class="rounded-xl border-slate-300 bg-white text-sm dark:border-slate-700 dark:text-white">
                <div class="flex gap-2 sm:col-span-2 lg:col-span-4 xl:col-span-7">
                    ...
                    <a href="{{ route('admin.whatsapp.index') }}" class="inline-flex h-10 items-center rounded-xl border border-slate-200 px-4 text-sm font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800">{{ __('Clear') }}</a>
                </div>
            </form>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($events as $event)
                            ...
                            <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/30">
                                <td class="px-4 py-3"><div class="font-mono font-bold text-slate-800" dir="ltr">{{ $event->sender_phone ?? '—' }}</div><div class="max-w-40 truncate text-xs text-muted">{{ $event->sender_name ?? '—' }}</div></td>
                                <td class="max-w-xs px-4 py-3"><div class="truncate text-slate-700" title="{{ $event->message_text }}">{{ \Illuminate\Support\Str::limit($event->message_text ?? __('No text payload'), 80) }}</div><div class="font-mono text-[10px] text-muted">{{ $event->external_message_id }}</div></td>
                                <td class="px-4 py-3 font-mono text-xs text-slate-600">{{ $event->message_type ?? '—' }}</td>
                                <td class="px-4 py-3 font-mono text-xs text-slate-600">{{ $event->event_type ?? '—' }}</td>
                                <td class="px-4 py-3 text-center font-mono text-slate-600">{{ $event->attempt_number ?? '—' }}</td>
                                ...
                                <td class="whitespace-nowrap px-4 py-3 text-xs text-slate-500">{{ $event->received_at?->timezone(config('app.timezone'))->format('Y-m-d H:i') ?? '—' }}</td>
                        ...
                        @empty
                            <tr><td colspan="9" class="px-6 py-16 text-center text-sm text-slate-500"><i class="fab fa-whatsapp mb-3 block text-3xl text-slate-300" aria-hidden="true"></i>{{ __('No webhook events match these filters.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            @if($events->hasPages())<div class="border-t border-slate-200 px-4 py-3">{{ $events->links() }}</div>@endif
        </section>
    </div>
