        ...
        <section class="overflow-hidden rounded-2xl border border-[#12124a] bg-[linear-gradient(160deg,#04041f_0%,#070740_55%,#070740_100%)] font-mono shadow-[0_28px_70px_rgba(2,2,20,0.6)]">
            ...
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-[#12124a] bg-[#04041f]/80 px-4 py-2.5 text-[13px] tracking-[0.14em] text-[#7c84b3]">
                ...
                <span class="uppercase">
                    ...
                    &middot; <span class="text-[#ff8a3d]">{{ $rangeDays }}D</span>
            ...
            <div class="flex gap-7 overflow-x-auto whitespace-nowrap border-b border-[#12124a] bg-[#04041f]/60 px-4 py-2.5 text-[13px] text-[#7c84b3]">
                ...
                <span class="uppercase tracking-[0.08em]">{{ __('Lifetime') }} <b class="ml-1 font-semibold text-[#ff8a3d]">{{ $currencyLabel }} {{ $fmt($totalRevenue) }}</b></span>
            ...
            <div class="grid xl:grid-cols-[1.65fr_1fr]">
                <div class="border-b border-[#12124a] p-5 xl:border-b-0 xl:border-r">
                    ...
                    <div class="mt-5 flex h-44 items-end gap-[3px]">
                        @foreach ($chartPoints as $point)
                            ...
                            @endphp
                            <div
                                class="flex-1 rounded-t-[2px] {{ $isPeak ? 'border-t-2 border-[#ffc9a3] bg-gradient-to-b from-[#e65c00]/80 to-[#78350f]/60 shadow-[0_0_14px_rgb(255 138 61 / 0.35)]' : 'border-t-2 border-[#ff8a3d]/80 bg-gradient-to-b from-[#b45309]/45 to-[#451a03]/30 hover:border-[#ffc9a3]' }}"
                    ...
                    <div class="mt-1.5 flex justify-between text-[11px] uppercase tracking-[0.12em] text-[#4a5288]">
                        ...
                        <span class="text-[#ff8a3d]">{{ __('Peak') }} {{ data_get($peakDay, 'label', '-') }} = {{ $currencyLabel }} {{ $fmt(data_get($peakDay, 'amount', 0)) }}</span>
                ...
                <div class="p-5">
                    <p class="text-xs uppercase tracking-[0.2em] text-[#ff8a3d]">{{ __('Top Movers') }} <span class="text-[#666fa3]">&mdash; {{ __('Products') }}</span></p>
                        ...
                        <tbody>
                            @forelse ($topProductsCollection->take(5) as $index => $product)
                                <tr class="border-b border-dotted border-[#12124a]">
                                    ...
                                    <td class="py-2 text-right font-semibold {{ $index === 0 ? 'text-[#ff8a3d]' : 'text-[#f2f3ff]' }}">{{ $fmt($product->revenue_total) }}</td>
                                </tr>
                            @empty
                    ...
                    <p class="mt-5 text-xs uppercase tracking-[0.2em] text-[#ff8a3d]">{{ __('Top Accounts') }}</p>
                    <table class="mt-2 w-full text-[13px]">
                        <tbody>
                                ...
                                <tr class="border-b border-dotted border-[#12124a]">
                                    <td class="max-w-0 truncate py-2 pr-3 text-[#c0c5e8]">
                                        ...
                                        @if (($customer->role ?? null) === 'dealer')
                                            <span class="ml-1 rounded-sm bg-[#ff8a3d]/15 px-1 text-[11px] font-semibold text-[#ff8a3d]">DLR</span>
                                        @endif
                                    </td>
                        ...
                        <tbody>
                            @forelse ($topProducts as $index => $product)
                                ...
                                <tr class="border-b border-dotted border-[#12124a] hover:bg-white/[0.03]">
                                    ...
                                    <td class="py-2.5 text-right align-top font-semibold {{ $index === 0 ? 'text-[#ff8a3d]' : 'text-[#f2f3ff]' }}">{{ $fmt($product->revenue_total) }}</td>
                                </tr>
                            @empty
                                    ...
                                    <td class="max-w-0 py-2.5 pr-3">
                                        <span class="block truncate text-[#c0c5e8]">
                                            @if (Route::has('admin.users.show') && auth()->user()?->can('manage-users'))
                                                <a href="{{ route('admin.users.show', $customer->id) }}" class="transition hover:text-[#ff8a3d] focus-visible:outline-none focus-visible:text-[#ff8a3d]">{{ $customer->name }}</a>
                                            @else
                                                {{ $customer->name }}
                                            @endif
                                            @if (($customer->role ?? null) === 'dealer')
                                        ...
                                        <span class="mt-1 block h-[4px] rounded-full bg-gradient-to-r from-[#e65c00] to-[#ff8a3d]/60" style="width: {{ $customerShare }}%"></span>
                                    </td>
                                    <td class="py-2.5 pr-3 text-right align-top text-[#7c84b3]">{{ number_format((int) $customer->order_count) }}</td>
                                    <td class="py-2.5 text-right align-top font-semibold {{ $index === 0 ? 'text-[#ff8a3d]' : 'text-[#f2f3ff]' }}">{{ $fmt($customer->revenue_total) }}</td>
                                </tr>
                            @empty
                            ...
                            @forelse ($topDealers as $index => $dealer)
                                ...
                                <tr class="border-b border-dotted border-[#12124a] hover:bg-white/[0.03]">
                                    ...
                                    <td class="max-w-0 py-2.5 pr-3">
                                        ...
                                        <span class="mt-1 block h-[4px] rounded-full bg-gradient-to-r from-[#e65c00] to-[#ff8a3d]/60" style="width: {{ $dealerShare }}%"></span>
                                    </td>
                                    <td class="py-2.5 pr-3 text-right align-top text-[#7c84b3]">{{ number_format((int) $dealer->order_count) }}</td>
                                    <td class="py-2.5 text-right align-top font-semibold {{ $index === 0 ? 'text-[#ff8a3d]' : 'text-[#f2f3ff]' }}">{{ $fmt($dealer->revenue_total) }}</td>
                                </tr>
                            @empty
            ...
            <div class="grid border-t border-[#12124a] xl:grid-cols-2">
                            ...
                            <tbody>
                                @foreach ($dailyCollection->reverse() as $row)
                                    ...
                                    <tr class="border-b border-dotted border-[#12124a] hover:bg-white/[0.03]">
                                        ...
                                        <td class="py-2 pr-3">
                                            <span class="block h-[7px] rounded-full {{ $isPeakRow ? 'bg-gradient-to-r from-[#ff8a3d] to-[#ffc9a3]' : 'bg-gradient-to-r from-[#b45309] to-[#e65c00]/70' }}" style="width: {{ $rowShare }}%"></span>
                                        </td>
                                        <td class="w-32 py-2 text-right font-semibold {{ $isPeakRow ? 'text-[#ff8a3d]' : 'text-[#f2f3ff]' }}">{{ $fmt($row['amount']) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                <div class="p-5">
                    <div class="flex items-baseline justify-between gap-3">
                        <p class="text-xs uppercase tracking-[0.2em] text-[#ff8a3d]">{{ __('Order Ledger') }} <span class="text-[#666fa3]">&mdash; {{ __('Recent Paid Orders') }}</span></p>
