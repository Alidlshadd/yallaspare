        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <div class="flex items-center gap-3">
                    <h2 class="text-2xl font-semibold text-slate-900">{{ __('Order') }} {{ $order->order_number }}</h2>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 space-y-6">
                    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
                        <div class="flex flex-col gap-3 border-b border-slate-200 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
                            <h3 class="text-sm font-semibold text-slate-900">{{ __('Order Summary') }}</h3>
                        ...
                        <div class="px-6 py-5 grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                ...
                                <p class="text-lg font-semibold text-slate-900">{{ $currencyLabel }} {{ number_format($total, $currencyDecimals) }}</p>
                            </div>
                            <div>
                                <p class="text-xs uppercase tracking-wide text-slate-500">{{ __('Payment') }}</p>
                                <p class="text-sm font-medium text-slate-900">{{ __(ucfirst(str_replace('_', ' ', (string) ($order->payment_method ?? '-')))) }}</p>
                                <span class="mt-1 inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $paymentMeta['class'] }}">
                                    {{ $paymentMeta['label'] }}
                                </span>
                            </div>
                            <div>
                                <p class="text-xs uppercase tracking-wide text-slate-500">{{ __('Items') }}</p>
                                <p class="text-sm font-medium text-slate-900">{{ __(':count items', ['count' => $order->items->count()]) }}</p>
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
                        <div class="border-b border-slate-200 px-6 py-4">
                            <h3 class="text-sm font-semibold text-slate-900">{{ __('Order Items') }}</h3>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                                <thead class="bg-slate-50">
                                    <tr>
                                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-slate-600">{{ __('Product') }}</th>
                                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-slate-600">{{ __('SKU') }}</th>
                                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-slate-600">{{ __('Qty') }}</th>
                                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-slate-600">{{ __('Unit Price') }}</th>
                                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-slate-600">{{ __('Subtotal') }}</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="px-6 py-10 text-center text-sm text-slate-500">{{ __('No items found for this order.') }}</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                ...
                <div class="space-y-6">
                    ...
                    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <h3 class="text-sm font-semibold text-slate-900">{{ __('Status Management') }}</h3>
                        <div class="mt-3">
                        ...
                        @if(empty($nextStatuses))
                            <p class="mt-2 text-xs text-slate-500">{{ __('This is a terminal state. No further transitions are allowed.') }}</p>
                        @else
                        ...
                        <div class="mt-6 border-t border-slate-200 pt-4">
                            <h4 class="text-xs font-semibold uppercase text-slate-500">{{ __('Timeline') }}</h4>
                            @if(\App\Models\Order::normalizedStatus((string) $order->status) === \App\Models\Order::STATUS_CANCELLED)
                                <div class="mt-3 flex items-center gap-2 text-sm text-rose-600">
                    ...
                    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        ...
                        @if($order->payments->isNotEmpty())
                            <div class="mt-4 overflow-hidden rounded-xl border border-slate-200">
                                @foreach($order->payments as $payment)
                                    <div class="border-b border-slate-200 px-3 py-3 text-xs last:border-b-0">
                                        <div class="flex items-center justify-between gap-3">
                                            <span class="font-semibold uppercase text-slate-700">{{ $payment->provider }}</span>
                                            <span class="rounded-full bg-slate-100 px-2 py-0.5 font-semibold text-slate-700">{{ __(ucfirst(str_replace('_', ' ', $payment->status))) }}</span>
                                        </div>
                                        <p class="mt-1 text-slate-500">{{ __('Provider ID: :id', ['id' => $payment->provider_payment_id ?: '-']) }}</p>
                        ...
                        <form method="POST" action="{{ route('admin.orders.update-payment', $order) }}" class="mt-4 space-y-3">
                            ...
                            >
                            <button type="submit" class="w-full rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 dark:text-slate-900 dark:hover:bg-slate-100">
                                {{ __('Update Payment') }}
                            </button>
                        </form>
                    </section>
                    ...
                    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <h3 class="text-sm font-semibold text-slate-900">{{ __('Internal Admin Notes') }}</h3>
                        ...
                        <div class="mt-5 space-y-3">
                            ...
                            @empty
                                <p class="text-sm text-slate-500">{{ __('No internal notes yet.') }}</p>
                            @endforelse
                        </div>
                    </section>
                    ...
                    @if($order->returnRequests->isNotEmpty())
                        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            <h3 class="text-sm font-semibold text-slate-900">{{ __('Return / Refund Requests') }}</h3>
                            <div class="mt-4 space-y-3">
                                @foreach($order->returnRequests as $returnRequest)
                                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                        <div class="flex items-center justify-between gap-3">
                                            <span class="text-sm font-semibold capitalize text-slate-900">{{ __(ucfirst(str_replace('_', ' ', (string) $returnRequest->type))) }}</span>
                                        ...
                                        <p class="mt-2 text-sm text-slate-700">{{ $returnRequest->reason }}</p>
                                        @if($returnRequest->admin_note)
                                            <p class="mt-2 text-xs text-slate-500">{{ __('Admin note:') }} {{ $returnRequest->admin_note }}</p>
                                        @endif
                    ...
                    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
                        <h3 class="text-sm font-semibold text-slate-900">{{ __('Totals') }}</h3>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-slate-500">{{ __('Subtotal') }}</span>
                            <span class="font-medium text-slate-900">{{ $currencyLabel }} {{ number_format($subtotal, $currencyDecimals) }}</span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-slate-500">{{ __('Shipping') }}</span>
                            <span class="font-medium text-slate-900">{{ $currencyLabel }} {{ number_format($shipping, $currencyDecimals) }}</span>
                        </div>
                        @if($discount > 0)
                            <div class="flex items-center justify-between text-sm">
                        ...
                        <div class="flex items-center justify-between border-t border-slate-200 pt-3 text-base">
                            ...
                            <span class="font-semibold text-slate-900">{{ $currencyLabel }} {{ number_format($total, $currencyDecimals) }}</span>
                        </div>
                    </section>

                    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
                        <h3 class="text-sm font-semibold text-slate-900">{{ __('Customer or Dealer') }}</h3>
                        <div>
                            <p class="text-xs uppercase text-slate-500">{{ __('Name') }}</p>
                            <p class="text-sm font-medium text-slate-900">{{ $order->user?->name ?? __('Guest customer') }}</p>
                        </div>
                        <div>
                            <p class="text-xs uppercase text-slate-500">{{ __('Email') }}</p>
                            <p class="text-sm text-slate-900">{{ $order->user?->email ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs uppercase text-slate-500">{{ __('Phone') }}</p>
                            <p class="text-sm text-slate-900">{{ $order->user?->phone ?? '-' }}</p>
                        </div>
                        <div>
                    ...
                    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
                        ...
                        <div>
                            <p class="text-xs uppercase text-slate-500">{{ __('City') }}</p>
                        ...
                        <div>
                            <p class="text-xs uppercase text-slate-500">{{ __('Phone') }}</p>
                            <p class="text-sm text-slate-900">{{ $order->delivery_phone ?? '-' }}</p>
                        </div>
                        @if(!empty($order->notes))
                            <div>
                                <p class="text-xs uppercase text-slate-500">{{ __('Notes') }}</p>
