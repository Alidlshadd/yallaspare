<x-app-layout>
    ...
    <div class="min-h-screen bg-slate-100 py-7 dark:bg-slate-950">
        <div class="mx-auto max-w-7xl space-y-5 px-4 sm:px-6 lg:px-8">
            @if(session('success'))
                <div class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300">{{ session('success') }}</div>
            @endif
            @if(session('error'))
                <div class="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-300">{{ session('error') }}</div>
            @endif
            @if($errors->any())
                <div class="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-300">{{ $errors->first() }}</div>
            @endif
            ...
            <div class="grid gap-5 lg:grid-cols-[340px_minmax(0,1fr)]">
                ...
                <main class="space-y-4">
                    <form method="GET" action="{{ route('admin.product-brands.index') }}" class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                                ...
                                <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-accent hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
                                    ...
                                    <div class="p-4">
                                        ...
                                        <details class="mt-3 group">
                                            <summary class="flex h-9 cursor-pointer list-none items-center justify-center gap-2 rounded-lg border border-slate-200 text-xs font-bold text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
                                                <i class="fas fa-pen"></i> {{ __('Edit Brand') }}
                                            </summary>
                                            <form method="POST" action="{{ route('admin.product-brands.update', $brand) }}" enctype="multipart/form-data" class="mt-3 space-y-3 rounded-xl bg-slate-50 p-3 dark:bg-slate-950" data-loading-form>
                                                @csrf
                                                @method('PUT')
                                        ...
                                        <form method="POST" action="{{ route('admin.product-brands.destroy', $brand) }}" class="mt-2" data-danger-form data-danger-title="{{ __('Delete Brand') }}" data-danger-description="{{ __('This brand will be removed permanently.') }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="h-9 w-full rounded-lg border border-rose-200 text-xs font-bold text-rose-600 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-45 dark:border-rose-500/30 dark:text-rose-300 dark:hover:bg-rose-500/10" @disabled($brand->products_count > 0) title="{{ $brand->products_count > 0 ? __('Remove product assignments before deleting this brand.') : __('Delete Brand') }}">
