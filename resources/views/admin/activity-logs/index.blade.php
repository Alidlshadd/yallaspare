<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 dark:text-slate-100">{{ __('Activity Logs') }}</h2>
            <p class="text-sm text-slate-500 mt-1 dark:text-slate-400">{{ __('Super admin audit trail') }}</p>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-sm text-slate-500 dark:text-slate-400">
                    {{ __('Showing') }} <span class="font-semibold text-slate-800 dark:text-slate-100">{{ $logs->count() }}</span>
                    {{ __('of') }} <span class="font-semibold text-slate-800 dark:text-slate-100">{{ number_format($logs->total()) }}</span>
                    @if(!empty($model))
                        · <span class="font-semibold text-slate-800 dark:text-slate-100">{{ __($model) }}</span>
                    @endif
                    @if(!empty($search))
                        · "{{ $search }}"
                    @endif
            ...
            <div class="grid gap-4 lg:grid-cols-[240px_minmax(0,1fr)] items-start">
                ...
                <div class="min-w-0 rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <div class="flex items-center gap-2 border-b border-slate-200 p-3 dark:border-slate-800">
                        ...
                        @if(!empty($search))
                            <a href="{{ route('admin.activity-logs.index', array_filter(['model' => $model])) }}" class="shrink-0 text-xs font-semibold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200">
                                {{ __('Clear') }} ✕
                            </a>
                        @endif
                    </div>
                                    ...
                                    <tr class="transition-colors hover:bg-slate-50/70 dark:hover:bg-slate-800/60">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-2.5">
                                                <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-100 text-[11px] font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                                                    {{ $causerInitial }}
                                                </span>
                                                <div class="min-w-0">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex min-w-[220px] items-center gap-2.5">
                                                ...
                                                <div class="min-w-0">
                                                    @if($target['url'])
                                                        <a href="{{ $target['url'] }}" class="block truncate font-semibold text-slate-800 hover:text-accent dark:text-slate-100 dark:hover:text-accent">{{ $target['name'] }}</a>
                                                    @else
                                            ...
                                            <div class="rounded-xl border border-slate-200 bg-slate-50/70 p-3 dark:border-slate-800 dark:bg-slate-900/50">
                                                <div class="mb-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4">
                                                    ...
                                                    <div class="rounded-lg border border-amber-200 bg-amber-50/60 px-3 py-2.5 dark:border-amber-400/20 dark:bg-amber-400/5">
                                                        ...
                                                        <div class="mt-1 truncate text-sm font-bold text-slate-800 dark:text-slate-100">{{ $target['name'] }}</div>
