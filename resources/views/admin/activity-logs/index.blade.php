<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">{{ __('Activity Logs') }}</h2>
            <p class="text-sm text-slate-500 mt-1">{{ __('Super admin audit trail') }}</p>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                    <p class="text-[11px] font-bold uppercase tracking-widest text-slate-500">{{ __('Total logged') }}</p>
                    <p class="mt-2 text-2xl font-bold tabular-nums text-slate-900">{{ number_format($totalCount) }}</p>
                ...
                <div class="rounded-xl border border-emerald-300/70 bg-white p-4 shadow-sm dark:border-emerald-400/35">
                    <p class="text-[11px] font-bold uppercase tracking-widest text-emerald-600">{{ __('Today') }}</p>
                    <p class="mt-2 text-2xl font-bold tabular-nums text-emerald-700">{{ number_format($todayCount) }}</p>
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-sm text-slate-500">
                    {{ __('Showing') }} <span class="font-semibold text-slate-800">{{ $logs->count() }}</span>
                    {{ __('of') }} <span class="font-semibold text-slate-800">{{ number_format($logs->total()) }}</span>
                    @if(!empty($model))
                        · <span class="font-semibold text-slate-800">{{ __($model) }}</span>
                    @endif
                    @if(!empty($search))
                        · "{{ $search }}"
                    @endif
            ...
            <div class="grid gap-4 lg:grid-cols-[240px_minmax(0,1fr)] items-start">
                ...
                <div class="min-w-0 rounded-xl border border-slate-200 bg-white shadow-sm">
                    <div class="flex items-center gap-2 border-b border-slate-200 p-3">
                        ...
                        @if(!empty($search))
                            <a href="{{ route('admin.activity-logs.index', array_filter(['model' => $model])) }}" class="shrink-0 text-xs font-semibold text-slate-500 hover:text-slate-700 dark:hover:text-slate-200">
                                {{ __('Clear') }} ✕
                            </a>
                        @endif
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                            ...
                            @forelse($logs as $log)
                                ...
                                <tbody x-data="toggle" class="border-b border-slate-100 last:border-b-0">
                                    <tr class="transition-colors hover:bg-slate-50/70 dark:hover:bg-slate-800/60">
                                        <td class="px-3 py-3">
                                            <button
                                                type="button"
                                                class="inline-flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:scale-105 hover:border-slate-300 hover:text-slate-700 hover:shadow dark:hover:border-slate-600 dark:hover:text-white"
                                                @click="toggle()"
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-2.5">
                                                <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-100 text-[11px] font-bold text-slate-600">
                                                    {{ $causerInitial }}
                                                </span>
                                                <div class="min-w-0">
                                                    <div class="truncate font-semibold text-slate-800">{{ $log->causer?->name ?? __('System') }}</div>
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="font-semibold text-slate-700">{{ \App\Support\ActivityLabelHelper::label($log->description, $log->subject_type) }}</div>
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex min-w-[220px] items-center gap-2.5">
                                                <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-amber-50 text-[11px] font-bold text-amber-700 ring-1 ring-inset ring-amber-200 dark:ring-amber-400/20">
                                                    {{ $target['initial'] }}
                                                </span>
                                                <div class="min-w-0">
                                                    @if($target['url'])
                                                        <a href="{{ $target['url'] }}" class="block truncate font-semibold text-slate-800 hover:text-accent dark:hover:text-accent">{{ $target['name'] }}</a>
                                                    @else
                                                        <div class="truncate font-semibold text-slate-800">{{ $target['name'] }}</div>
                                                    @endif
                                                    <div class="truncate text-[11px] text-muted dark:text-slate-500">
                                        ...
                                        <td class="whitespace-nowrap px-4 py-3">
                                            <div class="font-mono text-xs text-slate-500">{{ $log->created_at?->timezone(config('app.timezone'))->format('Y-m-d H:i:s') ?? '-' }}</div>
                                    ...
                                    <tr x-show="open" x-transition.opacity.duration.150ms>
                                        <td colspan="5" class="px-4 pb-4 pt-1">
                                            <div class="rounded-xl border border-slate-200 bg-slate-50/70 p-3 dark:bg-slate-900/50">
                                                <div class="mb-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4">
                                                    <div class="rounded-lg border border-slate-200 bg-white px-3 py-2.5">
                                                        ...
                                                        <div class="mt-1 truncate text-sm font-bold text-slate-800">{{ $log->causer?->name ?? __('System action') }}</div>
                                                        <div class="truncate text-[11px] text-slate-500">{{ $actorRole }}@if($log->causer?->email) · {{ $log->causer->email }}@endif</div>
                                                    </div>
                                                    <div class="rounded-lg border border-amber-200 bg-amber-50/60 px-3 py-2.5 dark:bg-amber-400/5">
                                                        <div class="text-[10px] font-bold uppercase tracking-widest text-accent/70 dark:text-accent/70">{{ __('Affected record') }}</div>
                                                        <div class="mt-1 truncate text-sm font-bold text-slate-800">{{ $target['name'] }}</div>
                                                        <div class="truncate text-[11px] text-slate-500">{{ $target['type'] }} #{{ $target['id'] ?? '-' }}@if($target['secondary']) · {{ $target['secondary'] }}@endif</div>
                                                    </div>
                                                    <div class="rounded-lg border border-slate-200 bg-white px-3 py-2.5">
                                                        <div class="text-[10px] font-bold uppercase tracking-widest text-muted dark:text-slate-500">{{ __('Exact time') }}</div>
                                                        <div class="mt-1 font-mono text-sm font-bold text-slate-800">{{ $log->created_at?->timezone(config('app.timezone'))->format('Y-m-d H:i:s') ?? '-' }}</div>
                                                        <div class="text-[11px] text-slate-500">{{ config('app.timezone') }}</div>
                                                    </div>
                                                    <div class="rounded-lg border border-slate-200 bg-white px-3 py-2.5">
                                                        <div class="text-[10px] font-bold uppercase tracking-widest text-muted dark:text-slate-500">{{ __('Event context') }}</div>
                                                        <div class="mt-1 font-mono text-sm font-bold text-slate-800">#{{ $log->id }} · {{ $log->event ?: $log->description }}</div>
                                                        <div class="truncate text-[11px] text-slate-500">{{ $log->batch_uuid ? __('Batch: :id', ['id' => $log->batch_uuid]) : __('Single operation') }}</div>
                                                    </div>
                                                </div>

                                                <div class="mb-2 flex items-center justify-between gap-3">
                                                    <div>
                                                        <div class="text-xs font-bold text-slate-700">{{ __('Changed fields') }}</div>
                                                        <div class="text-[11px] text-muted dark:text-slate-500">{{ __('Previous and new values recorded for this action.') }}</div>
                                                    </div>
                                                    @if($target['url'])
                                                        <a href="{{ $target['url'] }}" class="shrink-0 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm font-bold text-slate-600 hover:border-accent hover:text-accent dark:hover:border-accent/40 dark:hover:text-accent">{{ __('Open target') }}</a>
                                                    @endif
                                                </div>
                                                ...
                                                @else
                                                    <div class="max-h-64 space-y-1 overflow-y-auto pr-1">
                                                        @foreach($keys as $key)
                                                            ...
                                                            @endphp
                                                            <div class="grid grid-cols-[minmax(120px,180px),1fr] items-center gap-2 rounded-md border border-slate-200 bg-white/70 px-2 py-1.5 dark:bg-slate-900/40">
                                                                <div class="truncate text-[11px] font-semibold text-slate-600">{{ \App\Support\ActivityLogPresenter::fieldLabel((string) $key) }}</div>
                                                                <div class="flex flex-wrap items-center gap-2 text-[11px]">
                                                                    <span class="text-rose-600 line-through">{{ \App\Support\ActivityLogPresenter::value((string) $key, $oldVal) }}</span>
                                                                    <span class="text-muted dark:text-slate-500">→</span>
                                                                    <span class="text-emerald-600">{{ \App\Support\ActivityLogPresenter::value((string) $key, $newVal) }}</span>
                                                                    @if($delta !== null)
                                                ...
                                                <details class="group mt-3">
                                                    <summary class="cursor-pointer text-xs font-semibold text-slate-600 hover:text-slate-800 dark:hover:text-slate-100">
                                                        {{ __('View raw payload') }}
                                                    </summary>
