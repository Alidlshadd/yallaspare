...
<form method="POST" action="{{ $formAction }}" class="space-y-4" enctype="multipart/form-data">
    ...
    {{-- ═════════════ Category details ═════════════ --}}
    <div class="bg-white border border-slate-200/70 rounded-2xl p-5 sm:p-6 bento-shadow">
        <div class="flex items-center gap-2.5 mb-5">
            ...
            <div>
                ...
                <p class="text-[11px] text-slate-500">{{ __('Names in all three languages are required.') }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                @error('name_en')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
            </div>
            ...
            <div>
                ...
                @error('name_ar')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
            </div>
            ...
            <div>
                ...
                @error('name_ku')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
            </div>
            ...
            <div>
                ...
                @error('slug')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
            </div>
        </div>
        ...
        <div class="mt-4">
            ...
            @error('description')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
        </div>
    </div>
    ...
    <div class="bg-white border border-slate-200/70 rounded-2xl p-5 sm:p-6 bento-shadow">
        <div class="flex items-center gap-2.5 mb-5">
            ...
            <div>
                <h3 class="text-sm font-bold text-slate-900">{{ __('Category Image') }}</h3>
                <p class="text-[11px] text-slate-500">{{ __('Shown on the storefront category listing.') }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 {{ $isEdit && !empty($category->image) ? 'md:grid-cols-[minmax(0,1fr)_auto] md:items-start' : '' }}">
            ...
            @if($isEdit && !empty($category->image))
                <div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                    <div class="h-16 w-16 rounded-lg border border-slate-200 bg-white grid place-items-center overflow-hidden p-1.5">
                    ...
                    <div>
                        ...
                        <label class="mt-1.5 flex items-center gap-2 text-xs font-bold text-slate-600 cursor-pointer">
