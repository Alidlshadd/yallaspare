...
<form method="POST" action="{{ $formAction }}" class="space-y-4" enctype="multipart/form-data">
    ...
    <div data-animate="fade-up" class="max-w-xl mx-auto">
        <div data-motion-lift class="bg-white border border-slate-200/70 rounded-2xl overflow-hidden bento-shadow">
            ...
            <div class="p-3 text-center">
                <p class="text-[10.5px] font-bold text-slate-500">{{ __('Live preview — this is what visitors will see') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-4">
            ...
            {{-- ═════════════ Content ═════════════ --}}
            <div data-animate="fade-up" class="bg-white border border-slate-200/70 rounded-2xl p-5 sm:p-6 bento-shadow">
                <div class="flex items-center gap-2.5 mb-5">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-slate-900">{{ __('Popup Content') }}</h3>
                        <p class="text-[11px] text-slate-500">{{ __('English is required; Arabic and Kurdish fall back to English when empty.') }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        @error('title_en')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        ...
                        @error('title_ar')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        ...
                        @error('title_ku')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('description_en')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        ...
                        @error('description_ar')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        ...
                        @error('description_ku')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                </div>
            </div>
            ...
            <div data-animate="fade-up" class="bg-white border border-slate-200/70 rounded-2xl p-5 sm:p-6 bento-shadow">
                <div class="flex items-center gap-2.5 mb-5">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-slate-900">{{ __('Action Button (optional)') }}</h3>
                        <p class="text-[11px] text-slate-500">{{ __('Shown only when both a label and a link are set.') }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        @error('button_label_en')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        ...
                        @error('button_label_ar')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        ...
                        @error('button_label_ku')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div class="md:col-span-3">
                        ...
                        @error('button_url')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                </div>
            </div>
            ...
            <div data-animate="fade-up" class="bg-white border border-slate-200/70 rounded-2xl p-5 sm:p-6 bento-shadow">
                <div class="flex items-center gap-2.5 mb-5">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-slate-900">{{ __('Popup Image') }}</h3>
                        <p class="text-[11px] text-slate-500">{{ __('Covers the whole card; text sits on a dark gradient at the bottom. Without an image a navy brand background is used.') }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 {{ $isEdit && !empty($popup->image_path) ? 'md:grid-cols-[minmax(0,1fr)_auto] md:items-start' : '' }}">
                    <div>
                        ...
                        <input id="image" type="file" name="image" accept="image/*"
                               class="w-full rounded-xl border {{ $errors->has('image') ? $inputErr : $inputOk }} bg-slate-50 px-3 py-2.5 text-sm text-slate-900 file:mr-3 file:rounded-md file:border-0 file:bg-slate-200 file:px-3 file:py-1 file:text-xs file:font-bold">
                        @error('image')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>

                    @if($isEdit && !empty($popup->image_path))
                        <div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                            <div class="h-16 w-24 rounded-lg border border-slate-200 bg-white grid place-items-center overflow-hidden">
                                <img src="{{ $previewImageUrl }}" alt="{{ $popup->title_en }}" class="h-full w-full object-cover">
                            </div>
                            <div>
                                <div class="text-[10.5px] font-bold uppercase tracking-widest text-slate-500">{{ __('Current Image') }}</div>
            ...
            <div data-animate="fade-up" class="bg-white border border-slate-200/70 rounded-2xl p-5 sm:p-6 bento-shadow">
                <div class="flex items-center gap-2.5 mb-5">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-slate-900">{{ __('Schedule & Display Rules') }}</h3>
                        <p class="text-[11px] text-slate-500">{{ __('Leave dates empty for an open-ended popup.') }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        @error('starts_at')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        ...
                        @error('ends_at')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>

                    <div class="md:col-span-2">
                        <span class="{{ $labelClass }}">{{ __('Show On Pages') }}</span>
                        <div class="flex flex-wrap gap-x-5 gap-y-2 rounded-xl border border-slate-200 bg-slate-50 p-3.5">
                        ...
                        @error('pages')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('frequency')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            @error('frequency_days')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            ...
                            @error('delay_seconds')<p class="text-xs font-medium text-rose-600 mt-1.5">{{ $message }}</p>@enderror
                        </div>
                    </div>
