    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl space-y-5 px-4 sm:px-6 lg:px-8">
            ...
            @php
                ...
                if ($whatsappVisible) {
                    $summaryCards[] = ['label' => __('WhatsApp opt-ins'), 'value' => $stats['whatsapp_opt_in'], 'icon' => 'fab fa-whatsapp', 'tone' => 'green'];
                }
            @endphp
            <section class="grid gap-3 sm:grid-cols-2 {{ $whatsappVisible ? 'xl:grid-cols-5' : 'xl:grid-cols-4' }}" aria-label="{{ __('Messaging audience summary') }}">
                            ...
                            @foreach ($channels as $key => $channel)
                                ...
                                <article class="relative overflow-hidden rounded-2xl border p-5 {{ $cardClasses }}">
                                    <div class="flex items-start justify-between gap-4">
                                        <span class="inline-flex h-12 w-12 items-center justify-center rounded-2xl {{ $key === 'whatsapp' ? 'bg-green-500 text-white' : 'bg-info text-white' }} shadow-lg">
