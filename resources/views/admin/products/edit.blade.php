<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-slate-900">{{ __('Edit Product') }}</h2>
    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            @if(session('success'))
                <div class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800 dark:text-emerald-200">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <form id="productForm" method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" class="space-y-6" data-loading-form data-loading-button-text="Saving...">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <section class="bg-white rounded-2xl border border-slate-200 shadow-sm">
                            ...
                            <div class="p-6 space-y-4">
                                ...
                                <div id="productImagePreview" class="{{ $product->image ? 'flex' : 'hidden' }} items-center gap-4 rounded-xl border border-slate-200 bg-white p-4">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-slate-900">{{ __('Current Image') }}</p>
                                        <p class="text-xs text-slate-500">{{ __('Upload a new image to replace it.') }}</p>
                                    </div>
                                </div>
                    ...
                    <aside class="space-y-6">
                        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                            <div class="flex items-center justify-between gap-3">
                                <div>
                                    <p class="text-xs font-bold uppercase tracking-wide text-slate-500">{{ __('Product Analytics') }}</p>
                            ...
                            <div class="mt-4 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-500">
                ...
                <div class="sticky bottom-0 z-10 -mx-4 sm:-mx-6 lg:-mx-8 border-t border-slate-200 bg-white/90 px-4 py-4 backdrop-blur dark:bg-slate-900/80">
                    <div class="max-w-6xl mx-auto flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <p class="text-xs text-slate-500">{{ __('Changes are saved immediately after update.') }}</p>
