<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <style>
                .admin-products-table .product-name-clamp {
                    display: -webkit-box;
                    -webkit-line-clamp: 2;
            ...
            <div class="overflow-x-auto rounded bg-white shadow dark:bg-slate-900 dark:ring-1 dark:ring-slate-800">
                <table class="admin-products-table min-w-[720px] w-full table-fixed text-left text-sm md:min-w-0">
                    <colgroup>
                        <col class="w-[3%]">
                        <col class="w-[5%]">
                        <col class="w-[25%]">
                    ...
                    <thead class="bg-gray-100 text-xs uppercase tracking-wide text-gray-600 dark:bg-slate-800/70 dark:text-slate-300">
                        <tr>
                            ...
                            <th class="px-2 py-3 text-right font-semibold">{{ __('Dealer Price') }}</th>
                            <th class="px-2 py-3 font-semibold">
                    ...
                    <tbody>
                        @forelse($products as $product)
                            <tr class="border-t hover:bg-gray-50 dark:border-slate-800 dark:hover:bg-slate-800/60">
                                ...
                                <td class="px-2 py-3 align-middle">
                                    @if($product->image)
                                        <img
                                            ...
                                            alt="{{ $product->name }}"
                                            class="h-9 w-9 rounded-lg border border-slate-200 bg-white object-cover dark:border-slate-700 dark:bg-slate-950"
                                            loading="lazy"
                                        >
                                    @else
                                        <div class="flex h-9 w-9 items-center justify-center rounded-lg border border-dashed border-slate-300 bg-slate-50 text-slate-400 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-600">
                                ...
                                <td class="px-2 py-3 align-middle">
                                    <span class="block break-words font-mono text-[11px] leading-5 text-slate-700 dark:text-slate-300">{{ $product->sku }}</span>
                                </td>
                                <td class="px-2 py-3 align-middle">
                                ...
                                <td class="overflow-hidden px-2 py-3 align-middle text-right tabular-nums text-slate-800 dark:text-slate-200">
                                    <span class="block truncate whitespace-nowrap">{{ $currencyLabel }} {{ number_format($product->price, $currencyDecimals) }}</span>
                                </td>
                                <td class="overflow-hidden px-2 py-3 align-middle text-right">
                                    @if($product->dealer_price !== null)
                                        <span class="inline-flex max-w-full items-center truncate whitespace-nowrap px-2 py-1 text-xs rounded bg-indigo-100 text-indigo-700 font-semibold">
                                            {{ $currencyLabel }} {{ number_format($product->dealer_price, $currencyDecimals) }}
                                        </span>
                                        @if((float) $product->dealer_price >= (float) $product->price)
                                            <span class="mt-1 inline-flex items-center whitespace-nowrap px-2 py-1 text-xs rounded bg-amber-100 text-amber-800 font-semibold">
                                                {{ __('Margin warning') }}
                                            </span>
                                        @endif
                                    @else
                                ...
                                <td class="overflow-hidden px-2 py-3 align-middle">
                                    ...
                                    @if($product->stock_quantity === 0)
                                        <span class="mt-1 inline-block whitespace-nowrap px-2 py-1 text-xs rounded bg-red-200 text-red-800 font-semibold">
                                            {{ __('Out of stock') }}
                                        </span>
                                    @elseif($product->stock_quantity <= $lowStockThreshold)
                                        <span class="mt-1 inline-block whitespace-nowrap px-2 py-1 text-xs rounded bg-amber-100 text-amber-800 font-semibold">
                                            {{ __('Low stock') }}
                                        </span>
                                    @else
                                        <span class="mt-1 inline-block whitespace-nowrap px-2 py-1 text-xs rounded bg-emerald-100 text-emerald-700">
                                            {{ __('In stock') }}
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-2 py-3 align-middle text-right">
                                    ...
                                    <a href="{{ route('admin.products.edit', ['product' => $product, 'return_to' => request()->fullUrl()]) }}"
                                       class="inline-flex items-center justify-center rounded-md border border-blue-200 px-1.5 py-1 text-xs font-medium text-blue-700 transition hover:bg-blue-50 dark:border-blue-500/30 dark:text-blue-300 dark:hover:bg-blue-500/10">
                                        {{ __('Edit') }}
                                    </a>
                                    ...
                                    <form action="{{ route('admin.products.destroy', $product) }}"
                                        ...
                                        <button type="submit"
                                                class="inline-flex items-center justify-center rounded-md border border-red-200 px-1.5 py-1 text-xs font-medium text-red-700 transition hover:bg-red-50 dark:border-red-500/30 dark:text-red-300 dark:hover:bg-red-500/10">
                                            {{ __('Delete') }}
                                        </button>
                                    </form>
                                    </div>
