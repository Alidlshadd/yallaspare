        ...
        <div class="space-y-4">
                ...
                <div class="border-b border-slate-200/80 pb-6">
                    <div class="flex items-center gap-4">
                        @if (!empty($user->profile_photo_path))
                            ...
                            >
                        @else
                            <div class="inline-flex h-16 w-16 items-center justify-center rounded-full bg-primary text-xl font-semibold text-white">
                                {{ $avatarInitial }}
                            </div>
                        @endif
                        <div>
                            <p class="text-sm font-medium text-slate-500">{{ __('Account Hub') }}</p>
                            <h2 class="mt-1 text-2xl font-semibold tracking-[-0.03em] text-slate-950">{{ $user->name }}</h2>
                ...
                <div class="mt-6 space-y-4">
                    @foreach ([
                        ...
                        <a
                            ...
                            class="group block w-full rounded-3xl border border-slate-200/80 bg-slate-50 p-5 transition duration-200 hover:-translate-y-0.5 hover:border-primary/20 hover:bg-white hover:shadow-sm hover:shadow-slate-900/5 dark:hover:border-primary/30 dark:hover:bg-slate-900 dark:hover:shadow-black/20"
                        >
                            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                <div class="min-w-0">
                                    <p class="text-base font-semibold text-slate-950">{{ $section['title'] }}</p>
                                    <p class="mt-2 text-sm leading-6 text-slate-600">{{ $section['description'] }}</p>
                                </div>
                                <span class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition duration-200 group-hover:bg-slate-50 dark:bg-slate-900 dark:group-hover:bg-slate-800">
                                    {{ __('Manage') }}
                                    <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                        <path fill-rule="evenodd" d="M7.22 4.97a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 1 1-1.06-1.06L10.94 10 7.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" />
                                    </svg>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                <div class="rounded-3xl border border-slate-200/80 bg-white px-4 py-4 shadow-sm shadow-slate-900/5 dark:bg-slate-900 dark:shadow-black/10">
                    <p class="text-xs font-medium uppercase tracking-[0.14em] text-slate-500">{{ __('Orders') }}</p>
                ...
                <div class="rounded-3xl border border-slate-200/80 bg-white px-4 py-4 shadow-sm shadow-slate-900/5 dark:bg-slate-900 dark:shadow-black/10">
                    <p class="text-xs font-medium uppercase tracking-[0.14em] text-slate-500">{{ __('Addresses') }}</p>
                    <p class="mt-2 text-2xl font-semibold text-slate-900">{{ $addresses->count() }}</p>
                </div>
            </div>
        </div>
    </div>
