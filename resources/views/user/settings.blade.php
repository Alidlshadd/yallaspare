        ...
        <div class="space-y-4">
            ...
            <section class="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm shadow-slate-900/5 dark:bg-slate-900 dark:shadow-black/10 sm:p-8">
                <div class="border-b border-slate-200/80 pb-6">
                    <div>
                        <p class="text-sm font-medium text-slate-500">{{ __('Settings Hub') }}</p>
                        <h2 class="mt-1 text-2xl font-semibold tracking-[-0.03em] text-slate-950">{{ __('Manage each section separately') }}</h2>
                        <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{{ __('Each preference group now has its own dedicated page, so changes stay focused and easier to review.') }}</p>
                    </div>
                </div>
