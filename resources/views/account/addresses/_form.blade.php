...
<form method="POST" action="{{ $action }}" class="space-y-8">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div class="flex items-start gap-3">
            <span class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-[#070740]">
    ...
    <div class="space-y-6">
        <div class="grid gap-6 sm:grid-cols-2">
            ...
            <div>
                ...
                <input id="phone" name="phone" type="text" value="{{ old('phone', $address->phone) }}" placeholder="+964..." class="mt-2 block w-full rounded-xl border bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-transparent focus:outline-none focus-visible:ring-2 focus-visible:ring-[#070740] {{ $errors->has('phone') ? 'border-rose-300' : 'border-slate-200' }}">
        ...
        <div class="grid gap-6 sm:grid-cols-2">
            <div>
                ...
                <input id="country" name="country" type="text" value="{{ old('country', $address->country) }}" placeholder="{{ __('Iraq') }}" class="mt-2 block w-full rounded-xl border bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-transparent focus:outline-none focus-visible:ring-2 focus-visible:ring-[#070740] {{ $errors->has('country') ? 'border-rose-300' : 'border-slate-200' }}" required>
                @error('country')
                    <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input id="city" name="city" type="text" value="{{ old('city', $address->city) }}" placeholder="{{ __('Baghdad') }}" class="mt-2 block w-full rounded-xl border bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-transparent focus:outline-none focus-visible:ring-2 focus-visible:ring-[#070740] {{ $errors->has('city') ? 'border-rose-300' : 'border-slate-200' }}" required>
                @error('city')
                    <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div>
            ...
            <textarea id="notes" name="notes" rows="5" class="mt-2 block min-h-32 w-full rounded-xl border bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-transparent focus:outline-none focus-visible:ring-2 focus-visible:ring-[#070740] {{ $errors->has('notes') ? 'border-rose-300' : 'border-slate-200' }}">{{ old('notes', $address->notes) }}</textarea>
            @error('notes')
                <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <label class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            <input type="checkbox" name="is_default" value="1" @checked(old('is_default', $address->is_default)) class="h-4 w-4 rounded border-slate-300 text-[#070740] focus:ring-[#070740]/30">
    ...
    <div class="border-t border-slate-200/70 pt-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <a href="{{ route('account.addresses.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-medium text-slate-700 transition duration-200 hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#070740] focus-visible:ring-offset-2">
                    {{ __('Cancel') }}
                </a>
                <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-[#070740] px-5 py-2.5 text-sm font-semibold text-white transition duration-200 hover:opacity-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#070740] focus-visible:ring-offset-2">
