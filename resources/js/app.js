...
// My Garage picker: asks model, then year only when the model has a limited
// range, then saves the vehicle. Data arrives on data-* attributes because
// Blade's @js() compiles to JSON.parse() and the CSP build of Alpine will not
// reach a global from an expression.
Alpine.data('fitmentPicker', () => ({
    models: [],
    labels: {},
    step: 0,
    pickedId: null,
    ...
    init() {
        this.models = JSON.parse(this.$el.dataset.models);
        this.labels = JSON.parse(this.$el.dataset.labels);
    },

    get picked() { return this.models.find((m) => m.id === this.pickedId) || null; },

    // Offer only years this part was actually listed for.
    get years() {
        if (! this.picked || this.picked.from === null) return [];

        const out = [];
        for (let y = this.picked.to; y >= this.picked.from; y--) out.push(y);

        return out;
    },

    // Submitted with the form; empty rather than null so the field stays optional.
    get yearValue() { return this.year === null ? '' : this.year; },

    // Takes an id rather than the object: the buttons are rendered by Blade so
    // they exist without JS, and a number is all the CSP evaluator needs.
    pickModelById(id) {
        const model = this.models.find((m) => m.id === id);
        if (! model) return;

        this.pickedId = model.id;
        this.year = null;
        this.step = model.from === null ? 2 : 1;
    },
    ...
    get onModels() { return this.step === 0; },
    get onYears() { return this.step === 1; },
    get onResult() { return this.step === 2; },

    get isFit() {
        if (! this.picked) return false;
        if (this.picked.from === null) return true;

        return this.year !== null && this.year >= this.picked.from && this.year <= this.picked.to;
    },

    get isMiss() { return this.onResult && ! this.isFit; },

    get prompt() {
        if (this.onModels) return this.labels.askModel;
        if (this.onYears) return this.labels.askYear.replace(':model', this.picked.name);

        return this.labels.result;
    },

    get hint() {
        if (this.onModels) return this.labels.hintModel;
        if (this.onYears) return this.labels.hintYear;
        if (! this.picked) return '';

        return this.picked.from === null ? this.picked.name : this.picked.name + ' ' + this.year;
    },

    get statusText() { return this.isFit ? this.labels.compatible : this.labels.notCompatible; },

    get statusSub() {
        if (! this.picked) return '';
        if (this.picked.from === null) return this.labels.allYears;

        const range = this.picked.from + '–' + this.picked.to;

        return this.isFit
            ? this.labels.listed.replace(':range', range)
            : this.labels.listedOnly.replace(':model', this.picked.name).replace(':range', range);
    },
    ...
    get statusIconClass() {
        return this.isFit ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400';
    },

    stepClass(index) {
        if (this.step === index) return 'border-amber-400/60 bg-amber-400/15 text-amber-700 dark:text-amber-300';
        if (this.step > index) return 'border-emerald-500/40 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400';

        return 'border-app text-muted';
    },

    stepNameClass(index) { return this.step >= index ? 'text-app' : 'text-muted'; },

    modelClassById(id) {
        return this.pickedId === id
            ? 'border-amber-400/60 ring-1 ring-inset ring-amber-400/25'
            : 'border-app hover:-translate-y-0.5';
    },
}));
