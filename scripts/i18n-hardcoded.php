    ...
    foreach ($lines as $i => $line) {
        ...
        if (preg_match_all('/>([A-Za-z][A-Za-z0-9 ,\.\'\-:?!&]{4,80})</', $line, $m)) {
            foreach ($m[1] as $text) {
                ...
                if (str_contains($t, '__TRANS__') || str_contains($t, '__EXPR__') || str_contains($t, '__PHP__')) {
                    continue;
                }
                ...
                if (preg_match('/^[a-z\-]+:\s*[a-z0-9\-]+;?$/i', $t)) {
                    continue;
                }
        ...
        foreach (['placeholder', 'title', 'alt', 'aria-label'] as $attr) {
            if (preg_match_all('/\\b'.$attr.'\\s*=\\s*"([^"]{4,120})"/i', $line, $m)) {
                foreach ($m[1] as $text) {
                    $t = trim($text);
                    if (str_contains($t, '__TRANS__') || str_contains($t, '__EXPR__') || str_contains($t, '{{')) {
                        continue;
                    }
...
uksort($byFile, function ($a, $b) use ($byFile) {
    return count($byFile[$b]) <=> count($byFile[$a]);
});
