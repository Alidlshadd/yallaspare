...
foreach (array_slice($identKu, 0, 30) as $k) {
    echo "  - $k\n";
}
if (count($identKu) > 30) {
    echo '  ... (+'.(count($identKu) - 30)." more)\n";
}
...
foreach (['auth', 'validation', 'errors', 'invoice', 'user'] as $f) {
    ...
    if ($miAr) {
        echo '  missing in ar ('.count($miAr).'): '.implode(', ', array_slice($miAr, 0, 10)).(count($miAr) > 10 ? '...' : '')."\n";
    }
    if ($miKu) {
        echo '  missing in ku ('.count($miKu).'): '.implode(', ', array_slice($miKu, 0, 10)).(count($miKu) > 10 ? '...' : '')."\n";
    }
    if ($extraAr) {
        echo '  extra in ar ('.count($extraAr).'): '.implode(', ', array_slice($extraAr, 0, 10))."\n";
    }
    if ($extraKu) {
        echo '  extra in ku ('.count($extraKu).'): '.implode(', ', array_slice($extraKu, 0, 10))."\n";
    }
    ...
    if ($untrAr) {
        echo '  untranslated in ar ('.count($untrAr).'): '.implode(' | ', array_slice($untrAr, 0, 5))."\n";
    }
    if ($untrKu) {
        echo '  untranslated in ku ('.count($untrKu).'): '.implode(' | ', array_slice($untrKu, 0, 5))."\n";
    }
}
